...
struct JIT
{
	...
	jitNativeCodeHandler jitNativeCode;
	CompCompileHandler compCompile;
};
...
jitNativeCodeHandler originalJitNativeCodeMethod;
jitNativeCodeHandler myJitNativeCodeMethod;

CompCompileHandler originalcompCompileMethod;
CompCompileHandler mycompCompileMethod;
...
bool InitJit()
{
	...
	if(jitInstance)
	{
		...
		newMethod = &my_compileMethod;

		originalJitNativeCodeMethod = jitInstance->jitNativeCode;
		myJitNativeCodeMethod = &my_compileMethod;

		originalcompCompileMethod = jitInstance->compCompile;
		mycompCompileMethod = &my_compileMethod;
...
void SwapMethod (bool orig)
{
	if(jitInstance)
	{
		...
		jitNativeCodeHandler jitMethod = orig ? originalJitNativeCodeMethod :	myJitNativeCodeMethod;
		CompCompileHandler compMethod = orig ? originalcompCompileMethod : mycompCompileMethod;

		VirtualProtect(jitInstance, sizeof (ULONG_PTR), PAGE_READWRITE, &oldProtect);

		jitInstance->compileMethod = method;
		//jitInstance->jitNativeCode = jitMethod;
		//jitInstance->compCompile = compMethod;

		VirtualProtect(jitInstance, sizeof (ULONG_PTR), oldProtect, &oldProtect);

		isHooked = orig ? false : true;
	}
};
							   ...
							   ULONG  *nativeSizeOfCode)
{
	...
	CORINFO_CLASS_HANDLE methodHandle = comp->getMethodClass(info->ftn);

	// Get the method signature
	CORINFO_SIG_INFO methodSig;
	comp->getMethodSig(info->ftn,&methodSig,methodHandle);

	// Get the method def.
	mdMethodDef methodDef =	comp->getMethodDefFromMethod(info->ftn);


	ULONG32 cVars;
	ICorDebugInfo::ILVarInfo* vars;
	bool extendOthers;
	comp->getVars(info->ftn,&cVars,&vars,&extendOthers);

	// print to screen
	printf("\tJIT1 : \t0x%x %s.%s\n",(ULONG)info->ILCode,szClassName,szMethodName);	
	
	// Save bytes to file
	FILE *pfile = fopen(szMethodName,"wb");
	fwrite(info->ILCode,sizeof(info->ILCode),info->ILCodeSize,pfile);
	fflush(pfile);
	fclose(pfile);

	// call original method
	// not using the naked + jmp approach to avoid x64 incompatibilities and naked does not work with /clr
	int iRet = originalMethod(classthis, comp, info, flags, nativeEntry, nativeSizeOfCode);
...
void RetriveIl( ICorJitInfo *comp, CORINFO_METHOD_INFO *info)
{
	const char *szMethodName = NULL;
	const char *szClassName = NULL;
...
#define MAX_INSTR      100
	...
	//FILE *pfile = fopen(szMethodName,"wb");
	//fwrite(Calls,sizeof(Calls),4096,pfile);
	////fwrite(info->ILCode,sizeof(info->ILCode),info->ILCodeSize,pfile);
	//fflush(pfile);
	//fclose(pfile);

	//sprintf("%s::%s", CurMethod, Calls);

	//MessageBoxA(NULL, Calls, CurMethod, MB_ICONINFORMATION);
};
